...
select cities.city, regions.region, countries.country, countries.date from cities 
inner join regions on (regions.id = cities.id_region)
inner join countries on (countries.id = regions.id_country)
where city = 'Paris' or city = 'Moscow' or city like '%len%' and date >= '2019-05-18'
order by date asc;

-- добавим для удобства алиасы в названия "cities", "countries" и "regions"
select cities.city as cities, regions.region as regions, countries.country as countries, countries.date from cities 
inner join regions on (regions.id = cities.id_region)
inner join countries on (countries.id = regions.id_country)
where city = 'Paris' or city = 'Moscow' or city like '%len%' and date >= '2019-05-18'
order by date asc;
